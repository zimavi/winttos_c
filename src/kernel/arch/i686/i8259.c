...
enum {
    PIC_ICW4_8086           = 0x1,
    PIC_ICW4_AUTO_EOI       = 0x2,      // AKA: PIC_ICW4_AUTO_END_OF_INTERRUPT
    PIC_ICW4_BUFFER_MASTER  = 0x4,
    PIC_ICW4_BUFFER_SLAVE   = 0x0,
    PIC_ICW4_BUFFERED       = 0x8,
    PIC_ICW4_SFNM           = 0x10,     // AKA: PIC_ICW4_SPECIAL_FULLY_NESTED_MODE
} PIC_ICW4; 

enum {
    PIC_CMD_EOI             = 0x20,     // AKA: PIC_CMD_END_OF_INTERRUPT
...
void i8259_SendEOI(int irq)
{
    if(irq >= 8)
    {
        i686_outb(PIC2_COMMAND_PORT, PIC_CMD_SPECIFIC_EOI | irq-8);
        i686_outb(PIC1_COMMAND_PORT, PIC_CMD_SPECIFIC_EOI | 2);
    }
    else
    {
        i686_outb(PIC1_COMMAND_PORT, PIC_CMD_SPECIFIC_EOI | irq);
    }    
}
...
static const PICDriver g_Driver = {
    .Name = "i8259 PIC",
    .Probe = &i8259_Probe,
